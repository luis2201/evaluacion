...
document.getElementById("titulo-principal").innerText = "Evaluación Estudiante - Docente";
...
async function evaluar(idevaluacion)
{
    await axios.get('https://evaluacion.itsup.edu.ec/estudiante/heteroevaluacion/findheteroevaluaciondocente/' + idevaluacion)
    .then(function (res){
       let info = res.data[0];
    ...
    $.confirm({
		...
		buttons: {
			continuar: async function () {
                let idevaluacion = document.getElementById("idevaluacion").value;
				...
				await axios.post('https://evaluacion.itsup.edu.ec/estudiante/heteroevaluacion/insertar/', {
                    idevaluacion,
                    indicador1,
                    indicador2,
                    indicador3,
