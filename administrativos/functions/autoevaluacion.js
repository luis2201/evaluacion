...
document.getElementById("titulo-pagina").innerText = "Autoevaluación";
document.getElementById("titulo-principal").innerText = "Evaluación Administrativos";

//Class for default site
var mndashboard = document.getElementById('mnautoevaluacion');
mndashboard.classList.add("active", "bg-gradient-primary");

var myModal = new bootstrap.Modal(document.getElementById('autoevaluacionModal'), {
    keyboard: false
});

async function evaluar(idautoevaluacion)
{
    await axios.get(DIR + 'autoevaluacion/findautoevaluacionadministrativo/' + idautoevaluacion)
    .then(function (res){
       let info = res.data[0];
    ...
    document.getElementById("idautoevaluacion").value = idautoevaluacion;
    myModal.show();
}
...
function finalizar()
{
    ...
    $.confirm({
		...
		content: 'Desea finalizar el proceso y registrar la calificación del Administrativo?',
		type: 'blue',
		theme: 'modern',
		buttons: {
			continuar: async function () {
                let idautoevaluacion = document.getElementById("idautoevaluacion").value;
				...
				await axios.post(DIR + 'autoevaluacion/insert/', {
                    idautoevaluacion,
                    indicador1,
                    indicador2,
                    indicador3,
...
function validate()
{
    ...
    if(document.getElementById("totalindicador6").value == ""){
        flag = false;    
    }

    if(document.getElementById("totalindicador7").value == ""){
        flag = false;    
    }

    if(document.getElementById("totalindicador8").value == ""){
        flag = false;    
    }

    if(document.getElementById("totalindicador9").value == ""){
        flag = false;    
    }

    if(document.getElementById("totalindicador10").value == ""){
        flag = false;    
    }
